...
app = FastAPI(title='Fenix Sheets API', version='1.0.0')


def build_steamdt_search_url(item: str) -> str:
    # URL de busca igual ao navegador do SteamDT
    encoded = urllib.parse.quote_plus(item)
    return f'{STEAMDT_BASE}/mkt?search={encoded}'


def build_fenix_input_url(item: str) -> str:
    # Link que o usuário costuma colar no Fenix Engine
    steamdt_item_url = f'https://www.steamdt.com/en/cs2/{urllib.parse.quote(item, safe="")}'
    return steamdt_item_url
...
def extract_metrics_from_text(text: str):
    # Procura os blocos exibidos pelo Fenix Engine
    supply = None
    valuation_usd = None
...
def analyze_with_browser(item: str):
    ...
    with sync_playwright() as p:
        ...
        try:
            page.goto(FENIX_APP_URL, wait_until='networkidle', timeout=90000)

            # Tenta localizar o input principal do Streamlit
            input_box = page.locator('input').first
            input_box.wait_for(timeout=60000)
            input_box.fill(fenix_input_url)

            # Clica no botão Analyze / Analisar
            button = page.get_by_role('button', name=re.compile('analyze|analisar', re.I))
            ...
            supply, valuation_usd = extract_metrics_from_text(text)
            if supply is None or valuation_usd is None:
                raise HTTPException(
                    status_code=502,
                    detail='Não consegui ler Est. Supply ou Market Valuation. Verifique se o layout do Fenix mudou.'
                )
...
@app.get('/analyze')
def analyze(item: str = Query(..., min_length=3), x_fenix_token: str = Header(default='')):
